...
public class GameBoard {
	...
	**/
	//Costructor refactorizado
	GameBoard go > () = new GoCell(); addCell(go);
	
//Metodos refactorizados
  /**  public void addCard(Card card) {
        if(card.getCardType() instanceof Card.TYPE_CC) {
	...
	**/
   Cell addCell = (cell) > cells.add(cell);
   /**
	...
	**/
   PropertyCell addCell = (cell) > {int propertyNumber = getPropertyNumberForColor(cell.getColorGroup());
	colorGroups.put(cell.getColorGroup(), new Integer(propertyNumber + 1));
    cells.add(cell);}
/**
     Card drawCCCard() {
...
**/
    Cell<int> getCell = (newIndex)> {return (Cell)cells.get(newIndex);}
/**    
	public int getCellNumber() {
		return cells.size();
	}
**/
    int getCellNumber = () > return cells.size();
